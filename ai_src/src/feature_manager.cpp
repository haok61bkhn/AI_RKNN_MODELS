...
FeatureManager::FeatureManager(const std::string &save_path, int max_size,
                               int topk, int dim, float threshold) {
...
FeatureManager::FeatureManager(const std::string &config_path,
...
bool FeatureManager::SaveData() {
  alg_hnsw_->saveIndex(save_path_);
  return true;
}

bool FeatureManager::LoadData() {
  if (!CheckExistsFile(save_path_))
    SaveData();
  try {
...
int FeatureManager::GetNumDatas() { return alg_hnsw_->id_to_pid.size(); }

std::vector<std::string> FeatureManager::GetAllPids() {
...
bool FeatureManager::Insert(const std::vector<float> &feature,
...
bool FeatureManager::InsertMultiple(
...
bool FeatureManager::DeleteByPId(const std::string &pid) {
...
void FeatureManager::Dist2Percent(float &dist, float &percent) {
  if (dist <= 0.7) {
    percent = 1.0;
  } else if (dist <= 1.1) {
...
bool FeatureManager::DeleteById(const int id) {
  alg_hnsw_->markDelete(id);
  alg_hnsw_->id_to_pid.erase(id);
  return true;
}

bool FeatureManager::DeleteByIds(const std::vector<int> &ids) {
...
FeatureManager::Search(const std::vector<float> &feature, float min_score) {
...
FeatureManager::SearchMutilple(const std::vector<std::vector<float>> &features,
                               float min_score) {
