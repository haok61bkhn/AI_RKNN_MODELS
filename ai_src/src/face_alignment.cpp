...
cv::Mat FaceAligner::AlignFace(const cv::Mat& img,
                               std::vector<types::Point>& landmark) {
  cv::Mat aligned_face;
  float points_src[5][2] = {landmark[0].x, landmark[0].y, landmark[1].x,
                            landmark[1].y, landmark[2].x, landmark[2].y,
                            landmark[3].x, landmark[3].y, landmark[4].x,
                            landmark[4].y};
  ...
  aligned_face.create(112, 112, CV_32FC3);
  cv::Mat transfer_mat = transform(cv::Rect(0, 0, 3, 2));
  cv::warpAffine(img.clone(), aligned_face, transfer_mat, cv::Size(112, 112), 1,
                 0, 0);
  return aligned_face;
}
...
/*
References: "Least-squares estimation of
transformation parameters between two
point patterns", Shinji Umeyama, PAMI
1991, DOI: 10.1109/34.88573 Anthor: Jack
Yu
*/
